...
void hashtable_add(HASHTABLE *hashtable, char *key, char *value)
{
    ...
    // }
    hashtable[h] = list_add(hashtable[h], key, value);

    // printf("Added [%s] at address [%i]\n", key, h);
}

//  DETERMINE IF A REQUIRED STRING ALREADY EXISTS IN A GIVEN HASHTABLE
bool hashtable_find(HASHTABLE *hashtable, char *key)
{
    uint32_t h	= hash_string(key) % HASHTABLE_SIZE;     // choose list

    return list_find(hashtable[h], key);
}

LIST *hashtable_get(HASHTABLE *hashtable, char *key)
{
    uint32_t h	= hash_string(key) % HASHTABLE_SIZE;     // choose list
    if (list_find(hashtable[h], key)) {
        return hashtable[h];
...
// char *hashtable_getListItem(HASHTABLE *hashtable, char *key, int index) 
// {
//     uint32_t h	= hash_string(key) % HASHTABLE_SIZE;     // choose list
//     if (list_find(hashtable[h], key)) {
